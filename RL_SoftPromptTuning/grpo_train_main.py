...
def prepare_train_data(
    ...
    cnt = Counter([item["ground_truth"] for item in data])

    if args.resample_train:
        max_category = max(cnt.values())
        resample_set = []
        for each in cnt.keys():
            resample_set += random.choices([item for item in data if item['ground_truth'] == each],k=max_category - cnt[each])
        data += resample_set
...
def build_trainer(
    ...
    targs = GRPOConfig(
        output_dir=args.output_dir + f"{args.exp_name}-{og_data_size}",
        run_name=f"{args.exp_name}-{og_data_size}",
        logging_steps=1,
...
if __name__ == "__main__":
    ...
    trainer = build_trainer(model, train_dataset, args)
